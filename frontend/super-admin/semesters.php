<?php
// frontend/super-admin/semesters.php
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_POST['csrf_token']) && !empty($_SESSION['csrf_token']) && !hash_equals($_SESSION['csrf_token'], $submittedCsrf)) {
        ...
        if ($action === 'promote_cohort') {
            ...
            if ($res['success']) {
                $successMsg = $res['message'];
                $sysModel->logAudit($userId, 'SUPER_ADMIN_CREATE_SEMESTER', 'semesters', $res['semester_id'], "Super Admin created semester '{$data['name']}' (#{$data['semester_number']}) for Course ID #{$data['course_id']}.");
            } else {
                $errorMsg = $res['error'];
            }
            ...
            if ($res['success']) {
                $successMsg = $res['message'];
                $sysModel->logAudit($userId, 'SUPER_ADMIN_UPDATE_SEMESTER', 'semesters', $semId, "Super Admin updated semester '{$data['name']}' (#{$semId}).");
            } else {
                $errorMsg = $res['error'];
            }
            ...
            if ($res['success']) {
                ...
                $sysModel->logAudit($userId, 'SUPER_ADMIN_DELETE_SEMESTER', 'semesters', $semId, "Super Admin deleted semester '{$name}'.");
            } else {
                $errorMsg = $res['error'];
            }
        }
...
<head>
    ...
    <title>Academic Terms & Semesters - Super Admin - StudentOS AI</title>
            ...
            <div class="dashboard-content">
                ...
                <div class="card">
                    <div class="card-header" style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 14px;">
                        <h3><i class="fas fa-layer-group"></i> Master Semester Register</h3>
                ...
                <div class="modal-body">
                    <div class="form-group">
                        ...
                        <select name="course_id" id="create_sem_course" class="form-control" required onchange="autoSuggestSemName('create')">
                            ...
                            <?php foreach ($courses as $c): ?>
                                <option value="<?php echo (int)$c['id']; ?>" data-name="<?php echo htmlspecialchars($c['code']); ?>">
                                    <?php echo htmlspecialchars($c['name'] . ' (' . $c['code'] . ')'); ?>
                                </option>
                            <?php endforeach; ?>
                        </select>
